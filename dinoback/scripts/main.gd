...
func _ready() -> void:
	...
	if MainGlobals.take_pending_tutorial("dinoback") \
			or MainGlobals.take_auto_tutorial("dinoback", game.shown_instructions):
...
func start_tutorial() -> void:
	var tut: Script = load("res://dinoback/scripts/tutorial.gd")
	...
	game.begin_tutorial()
	# starting_level lives on DinobackG, not the game util, so the snapshot does not cover it.
	_tutorial_saved_level = DinobackG.starting_level_id
	DinobackG.starting_level_id = tut.tutorial_level_id()
	new_game()
	var runner: TutorialRunner = TutorialRunner.new()
	# The caption must stay off the card being judged and off the two answer buttons.
	runner.keep_clear = [
		func():
...
func _restore_tutorial_globals() -> void:
	if is_instance_valid($Level):
		...
		$Level.tutorial_hold_cards = false
	if _tutorial_saved_level >= 0:
		DinobackG.starting_level_id = _tutorial_saved_level
