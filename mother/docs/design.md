...
- The child body (and the active-mode mother body) is sampled at times snapped to a fixed grid (`t_base = floor(elapsed/dt_step)*dt_step`, `dt_step = step/scroll_px_per_ms`), not at fixed screen-x. Sampling at fixed screen-x makes sharp trail vertices (up→down with no hold) alias and jitter as they scroll, because the peak drifts between sample points each frame. Snapping the sample times keeps each vertex's neighbouring samples constant frame-to-frame, so the trail scrolls smoothly. The guided mother path avoids this differently — it inserts exact vertices at the known phase-transition times (`_extra_xs`).
