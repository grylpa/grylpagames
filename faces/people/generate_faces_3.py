...
HAIR_STYLE = ["straight hair", "wavy hair", "curly hair", "slightly messy hair", "sleek hair"]
HAIR_COLOR = ["brown hair", "black hair", "blonde hair", "dark blonde hair", "salt-and-pepper hair", "gray hair"]
...
def build_prompt(name, gender, ethnicity, age=None):
    ...
    if gender == "male" and random.randint(0, 3) == 0:
        ...
        beard = random.choice(beard_options) + ", "
        
    attrs = [
        random.choice(HAIR_LEN),
        random.choice(HAIR_STYLE),
        random.choice(HAIR_COLOR),
        random.choice(EYE_COLOR),
        # random.choice(BG),
        random.choice(WARDROBE),
        # random.choice(POSE),
        # random.choice(LIGHT),
        # random.choice(LENS),
        # random.choice(EXPRESS),
    ]
    attr_str = ", ".join(attrs)
    ...
    prompt = (
        ...
        f"{glasses}{beard}{attr_str}neutral background, soft diffused lighting, shoulder framing, centered, "
