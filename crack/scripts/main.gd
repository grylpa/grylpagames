...
const POS_MODE: int = 8
const POS_LEVEL: int = 9
func _ready() -> void:
	game = CrackG.game
	...
	game.score_columns = ["didwin", "aborted", "duration_min", "cycles_opened", "mode", "level"]
	# THE LEVEL IS THE PAIR (duration, mode) -- see CrackG.level_id. This game has no levels of its
	...
	for m: int in range(1, 16):
		for mode: int in CrackG.mode_count():
			level_names[CrackG.level_id(m, mode)] = CrackG.level_label(m, mode)
