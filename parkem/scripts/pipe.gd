...
func set_blocked(_blocked:bool):
	blocked = _blocked
	$Bricks.visible = blocked
	
func set_rot(board):
	var p = board_pos
	var r = p.x+1 < ParkemG.game.board_size.x and board[p.y][p.x+1].ispipe
	var d = p.y+1 < ParkemG.game.board_size.y and board[p.y+1][p.x].ispipe
