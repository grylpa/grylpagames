...
│   └── level_defs.gd     # Autoloaded as SortingRobotsLevelDefs
└── scenes/
    ├── main.tscn
    └── level.tscn
```
...
- `SortingRobotsLevelDefs` — `level_defs.gd`; provides `LEVELS`, `LEVEL_PROGRESSION_ORDER`, `get_level(id)`, `level_names()`, `id_to_index(id)`
...
| ID | Name   | Left       | Right       | Hide after | Rounds | Window dur | Belt speed |
|----|--------|-----------|-------------|-----------|--------|------------|------------|
