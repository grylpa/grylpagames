...
var game: GenericGameUtil = GenericGameUtil.new("Sorting Robots", "sortingrobots", 0, 10, 0, 0)
...
func _progression() -> Array:
	var order: Array = SortingRobotsLevelConfig.LEVEL_PROGRESSION_ORDER.duplicate()
	if not order.is_empty() and int(order[order.size() - 1]) == REPEAT_LAST:
		order.remove_at(order.size() - 1)
	return order

func _repeats_last() -> bool:
	var order: Array = SortingRobotsLevelConfig.LEVEL_PROGRESSION_ORDER
...
func pass_pct_for(level_id: int) -> int:
	for lv in SortingRobotsLevelConfig.LEVELS:
