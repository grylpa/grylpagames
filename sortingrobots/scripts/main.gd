...
func _ready() -> void:
	...
	if MainGlobals.take_pending_tutorial("sortingrobots") \
			or MainGlobals.take_auto_tutorial("sortingrobots", game.shown_instructions):
...
func start_tutorial() -> void:
	var tut: Script = load("res://sortingrobots/scripts/tutorial.gd")
	...
	game.begin_tutorial()
	# starting_level lives on SortingRobotsG, not the game util, so the snapshot does not cover it.
	_tutorial_saved_level = SortingRobotsG.starting_level_id
	SortingRobotsG.starting_level_id = tut.tutorial_level_id()
	new_game()
	var runner: TutorialRunner = TutorialRunner.new()
	# The caption must not cover either rule label or the framed item it is talking about.
	runner.keep_clear = [
		func(): return $Level.tutorial_rule_label(0),
		func(): return $Level.tutorial_rule_label(1),
		func(): return $Level.tutorial_window_panel(),
	]
...
func _restore_tutorial_globals() -> void:
	$Level.tutorial_hold_belts = false
	$Level.tutorial_want_truth = -1
	if _tutorial_saved_level >= 0:
		SortingRobotsG.starting_level_id = _tutorial_saved_level
