...
func _ready() -> void:
	...
	main_menu.add_option_entry(1, "Starting level", RlmLevelConfig.level_names())
	refresh_menu()
	show_main_menu()
	...
	for lvl in RlmLevelConfig.LEVELS:
...
func _on_menu_option_changed(id: int, idx: int) -> void:
	if id == 1:
		RlmadnressG.starting_level_id = RlmLevelConfig.LEVELS[idx]["id"]
		RlmadnressG.save_settings()
...
func add_score_line_vals(score_row: Array) -> Array:
	...
	if score_row.size() > POS_SCORE_LEVEL_ID:
		var lvl: Dictionary = RlmLevelConfig.get_level(score_row[POS_SCORE_LEVEL_ID])
...
func refresh_menu() -> void:
	main_menu.update_option(1, RlmLevelConfig.id_to_index(RlmadnressG.starting_level_id))
