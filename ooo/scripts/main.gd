...
func _ready() -> void:
	...
	main_menu.add_entry(1, "Level", 1, OooLevelConfig.LEVELS.size(), false)
	refresh_menu()
	show_main_menu()
	$Help.set_texts({
		"N": "New game",
...
func _maybe_start_tutorial() -> void:
	if MainGlobals.take_pending_tutorial("ooo") \
			or MainGlobals.take_auto_tutorial("ooo", game.shown_instructions):
...
func start_tutorial() -> void:
	var tut: Script = load("res://ooo/scripts/tutorial.gd")
	...
	game.begin_tutorial()
	_tutorial_saved_level = OooG.starting_level
	OooG.starting_level = tut.tutorial_level_id()
	new_game()
	var runner: TutorialRunner = TutorialRunner.new()
	# The caption must stay off the model while it is being memorized, and off the lineup the
...
func _restore_tutorial_globals() -> void:
	...
	if _tutorial_saved_level >= 0:
		OooG.starting_level = _tutorial_saved_level
...
func refresh_menu():
	main_menu.update_val(1, OooG.starting_level)

func on_menu_slider_changed(id: int, val: float) -> void:
	if id == 1:
		OooG.starting_level = roundi(val)
		OooG.save_settings()
