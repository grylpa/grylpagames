...
Buoy has no levels: nothing gets harder and nothing is unlocked. What it has are two
...
of the same pattern. Before this the sessions were grouped on duration alone, so the charts drew one
series over everything and the baseline compared one pattern against another.

The level id is the PAIR, encoded by `UdbrG.level_id(duration_min, mode)` as
`duration * 100 + mode`. One integer, because the scores window groups on a single column.
`UdbrG.level_label()` names it (`5 min - 4-2-4-2`) and `UdbrG.mode_label()` handles the two modes
