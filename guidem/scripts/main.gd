...
func _ready() -> void:
	...
	main_menu.add_entry(1, "Level", 1, GuidemLevelConfig.LEVELS.size(), false)
	refresh_menu()
	show_main_menu()
	$Help.set_texts({
		"F":"Faster",
	...
	if MainGlobals.take_pending_tutorial("guidem") \
			or MainGlobals.take_auto_tutorial("guidem", game.shown_instructions):
...
func start_tutorial() -> void:
	var tut: Script = load("res://guidem/scripts/tutorial.gd")
	...
	game.begin_tutorial()
	# starting_level lives on GuidemG, not the game util, so the snapshot does not cover it.
	_tutorial_saved_level = GuidemG.starting_level
	GuidemG.starting_level = tut.tutorial_level_id()
	new_game()
...
func _restore_tutorial_globals() -> void:
	if _tutorial_saved_level >= 0:
		GuidemG.starting_level = _tutorial_saved_level
...
func refresh_menu():
	main_menu.update_val(1, GuidemG.starting_level)

func on_menu_slider_changed(id, val):
	if id == 1:
		GuidemG.starting_level = roundi(val)
