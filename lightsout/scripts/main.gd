...
func _ready() -> void:
	...
	main_menu.add_entry(1, "Level", 1, LightsoutLevelConfig.LEVELS.size(), false)
	refresh_menu()
	show_main_menu()
	$Help.set_texts({
		"F": "Faster",
	...
	game.set_instructions("Light out", 
		"Move your agent to its target position\n\n" +
		"Avoid running into bombs in junctions or in walls\n\n" +
		"Remember your path and the position of obstacles\n\n" +
		"You cannot move while the lights are on")
	if !game.shown_instructions:
		game.show_instructions(self)
		LightsG.save_settings()
...
func refresh_menu():
	main_menu.update_val(1, LightsG.starting_level)

func on_menu_slider_changed(id, val):
	if id == 1:
		LightsG.starting_level = roundi(val)
